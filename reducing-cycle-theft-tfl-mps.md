...
2009/10から2010/11に、
認知自転車盗は23,317件から22,064件へ、
5.4％減少しました。

この5.4％は、複合施策全体の前後変化です。

各対策の実装量や対象地点ごとの変化を併せて記録すると、
...
このため、
ホットスポット 対象設定が対策の一部になりました。

## 5．対策｜30人の自転車盗対策タスクフォース

Transport for Londonが資金を提供し、
Metropolitan Police Service内に
30人の自転車盗対策タスクフォース（CTF）を設置。

London全体をまたぐ問題として、
boroughごとのバラバラな対応ではなく、
専門チームによる統一的な対策を採用しました。
...
同時期に、
cycle tripsが約14％増えたとの初期統計も報告されています。
...
実施期間中、
Tower Hamletsで前年同期比16％減と報告されています。
...
内訳例：
- 自転車盗対策タスクフォース 警察官30人：£1,200,000
- TfL staff：£33,500
- marking kits：£13,000
- leaflets：£5,000
- 駐輪 stickers：£1,255
...
5.4％減少を施策の因果効果と確定できないため、
この£765,583も
厳密な費用便益 estimateとは扱いません。
...
2．場所、対象、犯人の3方向から対策を組み合わせる。

3．ホットスポット・ポリシングは対策の一部であり、それだけが施策ではない。

4．登録・回収・検挙と盗難件数を別評価指標・結果（アウトカム）で管理する。

5．複合施策では「どれが効いたか」が分かりにくくなるため、評価設計を先に作る。

6．5.4％減を「施策の効果5.4％」とは表現しない。

<!-- STATS_DETAIL_START -->

## 統計を詳しく見る

**対象：RED-02 TfL × Metropolitan Police**
...
London全体の複合施策について、
単一の独立比較群を用いた因果効果推定ではない。

統一した95％CI・p値は報告されていない。
