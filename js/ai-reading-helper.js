...
const SOURCE_SCOPE_RULES = [
  "回答は、指定された原著で確認できる内容を根拠とする",
  "AI自身の一般知識や、他の研究・Web情報を使って回答を補完しない",
  "原著に記載されていない解釈、理由、メカニズム、実務上の意味を推測して追加しない",
  "原著の「研究結果」と「著者による解釈」を区別する",
  "著者自身が推測・可能性として述べている内容は断定せず、「著者は〜の可能性を指摘しています」などと表現する",
  "質問の一部について原著から確認できない場合は、その部分について「この原著からは確認できません」とする",
  "原著そのものを確認できない場合は、タイトル・要約・一般知識などから回答を推測せず、原著を確認できないことを伝える"
];
...
function buildPrompt(question){
  ...
  const scopeRules = SOURCE_SCOPE_RULES
    .map(rule => `・${rule}`).join("\n");

  const rules = COMMON_READING_RULES
    .map(rule => `・${rule}`).join("\n");
...
export function renderAiReadingBlock(config){
  ...
  return `
    <section class="ai-reading" data-ai-reading aria-labelledby="aiReadingTitle">
      <div class="ai-reading-head">
        <div class="ai-reading-kicker">原著読解補助</div>
        <h2 id="aiReadingTitle">AIで原著を読み解く</h2>
        <p>この資料について、AIで確認したい内容を選んでください。</p>
        <p>
          原著を確認するための質問文を作成します。
          作成された質問文をコピーし、
      ...
      <p class="ai-reading-status" data-ai-reading-status role="status" aria-live="polite"></p>
      ...
      <script type="application/json" data-ai-reading-config
